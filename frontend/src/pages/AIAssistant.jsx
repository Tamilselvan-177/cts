...
import {
  ...
  BarChart2, X, RefreshCw, Sparkles, MessageSquare, Plus, Clock
} from "lucide-react";
import { askAssistant, getChatSessions, getSessionHistory } from "../services/api";
...
export default function AIAssistant() {
  ...
  const loadSession = async (sessionId) => {
    try {
      ...
      if (res.session) {
        ...
        const loadedCharts = [];
        res.session.messages.forEach(h => {
          loadedMessages.push({ role: "user", content: h.query });
          loadedMessages.push({
            role: "assistant",
            content: h.response,
            agentData: h.agent_data,
            visualization: h.visualization
          });
          if (h.visualization) loadedCharts.push(h.visualization);
        });
        setMessages(loadedMessages);
        setCharts(loadedCharts.reverse().slice(0, 10));
  ...
  const sendMessage = async (text) => {
    ...
    try {
      const res = await askAssistant(query, vizEnabled, activeSessionId);
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <div className="flex items-center gap-2 shrink-0 relative">
            ...
            <div className="relative">
              ...
              {historyOpen && (
                <div className="absolute right-0 mt-2 w-64 glass rounded-xl border border-white/10 shadow-2xl z-50 overflow-hidden flex flex-col max-h-64">
                  <div className="p-3 border-b border-white/10 flex justify-between items-center bg-black/20">
                    <span className="text-xs font-semibold text-white">Previous Chats</span>
                    <button onClick={() => setHistoryOpen(false)}><X size={14} className="text-slate-400 hover:text-white" /></button>
                  </div>
                  <div className="overflow-y-auto p-2 space-y-1">
                    {sessions.map(s => (
                      <button key={s.id} onClick={() => loadSession(s.id)} className="w-full text-left text-xs text-slate-300 hover:text-white hover:bg-white/10 p-2 rounded-lg truncate transition-colors">
                        {s.title}
                      </button>
                    ))}
                    {sessions.length === 0 && <div className="text-xs text-slate-500 p-2 text-center">No history yet</div>}
                  </div>
                </div>
              )}
            </div>
